...
def detention_notify(update, context):
    context.bot.send_message(chat_id=update.message.chat.id,
                             text=config.STREET_DETENTION_NOTIFY,
                             reply_markup=menu.show(menu=config.DETENTION),
                             parse_mode=ParseMode.MARKDOWN)
    return state.STREET_DETENTION


def retention_notify(update, context):
    context.bot.send_message(chat_id=update.message.chat.id,
                             text=config.STREET_RETENTION_NOTIFY,
                             reply_markup=menu.show(menu=config.RETENTION),
                             parse_mode=ParseMode.MARKDOWN)
    return state.STREET_RETENTION


def to_sell_goods(update, context):
    context.bot.send_message(chat_id=update.message.chat_id,
                             text="",
                             reply_markup=menu.show(menu=config.SELL_GOODS),
                             parse_mode=ParseMode.MARKDOWN)
    ...
    try:
        ...
        if high < config.MIN_HIGH_FOR_STREET:
            ...
            high_in_balance = sql.get_from_table(db_path=config.DB_PATH,
                                                 ...
                                                 table="balance",
                                                 field="high")
            money_in_balance = sql.get_from_table(db_path=config.DB_PATH,
                                                  telegram_id=chat_id,
                                                  table="balance",
                                                  field="money")
            if (high_in_balance < high) or (money_in_balance < config.MIN_MONEY_FOR_STREET):
                context.bot.send_message(chat_id=chat_id,
                                         text=config.INSUFFICIENT_FUNDS,
                                         reply_markup=menu.show(menu=config.BACK),
                                         parse_mode=ParseMode.MARKDOWN)
                return state.STREET_ENTER_HIGH
            else:
...
def street(update, context):
    context.bot.send_message(
        chat_id=update.message.chat.id,
        text=config.STREET_START_TEXT.format(high=context.user_data["high"]),
        reply_markup=menu.no_menu(),
        parse_mode=ParseMode.MARKDOWN)
...
def escape(update, context):
    ...
    if choices(population=config.SECOND_EVENT, weights=weights, k=1) == ["escape"]:
        sql.escape(db_path=config.DB_PATH,
                   telegram_id=update.message.chat.id,
                   money=context.user_data["money_for_escape"])
        ...
        high = context.user_data["high"]
        context.job_queue.run_once(
            callback=street_second_event,
            when=1.5,
            context=(update.message.chat_id, money_retention, high)
        )
        return state.STREET_RETENTION
